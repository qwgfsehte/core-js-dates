...
function getNextFriday(date) {
  ...
  if (currentDay < 5) {
    nextFriday.setUTCDate(date.getUTCDate() + (5 - currentDay));
  }
  if (currentDay > 5) {
    nextFriday.setUTCDate(date.getUTCDate() + (5 + (7 - currentDay)));
  }
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const oneDate = new Date(dateStart);
  const twoDate = new Date(dateEnd);
  return (Date.parse(twoDate) - Date.parse(oneDate)) / 1000 / 60 / 60 / 24 + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const two = new Date(period.start);
  const three = new Date(period.end);
...
function getWeekNumberByDate(date) {
  ...
  const differenceDate = currentDate - startYearDate;
  return Math.ceil(
    (differenceDate / (24 * 60 * 60 * 1000) + startYearDate.getDay() + 1) / 7
  );
}
